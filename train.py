...
scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
    optimizer, len(train_loader) * 10, T_mult=2, eta_min=0, last_epoch=-1
)
...
def validation_loop(validation_data_loader, net):
    ...
    if iou > max_valid_iou:
        ...
        save_path = os.path.join(
            "checkpoints", "{}_{}_{}.cp".format(RUNNAME, i, iou.item())
        )
...
def train_validation_loop(
    net, optimizer, scheduler, train_loader, valid_loader, num_epochs, cur_epoch
):
    global running_loss
    global running_iou
    global running_count
